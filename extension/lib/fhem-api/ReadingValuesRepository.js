class FhemApiReadingValuesRepository {
  ...
  async saveDeviceReadingsToStorage (cachePrefix, timestamps, deviceReadingsResponses) {
    ...
    for (let i = 0; i < timestamps.length; ++i) {
      saveStoragePromises.push(this._storage.set(cachePrefix + timestamps[i].toString(), deviceReadingsResponses[i]))
    }
  ...
  async getStorageReadResults (cachePrefix, timestamps) {
    ...
    for (let i = 0; i < timestamps.length; ++i) {
      readStoragePromises.push(this._storage.get(cachePrefix + timestamps[i].toString()))
    }

    return Promise.all(readStoragePromises)
  }
  ...
  async getRequests (device, reading, timestamps) {
    ...
    for (let timestamp of timestamps) {
      requestPromises.push(this._client.getReadingValues(device, reading, timestamp, timestamp + 86400000))
    }
